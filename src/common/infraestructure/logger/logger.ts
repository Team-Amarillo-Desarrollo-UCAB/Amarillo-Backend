...
export class NativeLogger implements ILogger {
  ...
  SuccessLog(logData: LoggerDto): void {
    this.logger.log(
      `Execution of: ${logData.name} successful, made by user ${logData.userId} with data ${logData.data}`,
    );
  }

  FailLog(logData: LoggerDto): void {
    this.logger.error(
      `Execution of: ${logData.name} failed, made by user ${logData.userId} with data ${logData.data}`,
    );
  }
}
